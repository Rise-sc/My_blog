...
​	判断Query相关的上下文是否排在Contexts的前面, 之后将Query和Contexts向量化, Contexts切块后与Query计算余弦相似度,高于某个阈值后(如0.7), 则判定为相关, 否则反之, 也可使用NLI来进行,但速度较慢,  后续将判定好的Contexts计算在整个Contexts的纯度 , 纯度再除以contexts的数量, 得到其片段相关性. 
